...
describe("Test Datepicker via webdriveruni", () => {
  ...
  it("Select date-picker from date picker", () => {
    ...
    var date = new Date();
    date.setDate(date.getDate() + 250); //+ sign will add an extra day to the current date
    ...
    function selectMonthAndYear() {
      cy.get(".datepicker-dropdown")
        ...
        .first()
        .then(($datePicker) => {
          cy.log("The future date is: " + $datePicker.text()); //logs the current month
          /**
           * will log all months until it reaches
           * the desired date (number of days added to the current date)
           */
